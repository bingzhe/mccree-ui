...
export const type = variant({
    ...
    variants: {
        fade: css`
            ...
            ${({ visible }: StyledContainerProps): string => (visible ? `opacity: 1;` : `opacity:0;visibility:hidden;`)}
        `,
        zoom: css`
        ...
        slide: css`
            transition: ${createBaseTransition(["transform", "visibility"])};
            ${({ visible }: StyledContainerProps): string => (visible ? `transform: none` : `transform:translateY(100vh);visibility:hidden;`)}
        `,
        collapse: css`
            overflow: hidden;
