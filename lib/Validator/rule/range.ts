 ...
 */
function range(rule: Rule, value: any, source: any, errors: any, options: any): void | boolean {
    ...
    } else if (max && !min && val > (rule.min as number)) {
        errors.push(util.format(options.messages[key].max, rule.field, rule.max));
    } else if (min && max && (val < (rule.min as number) || val > (rule.min as number))) {
